...
`references/viewport-and-base.css` is the single source of truth for viewport fitting, responsive breakpoints, grid behavior, image constraints, and reduced-motion defaults.

When generating a deck, copy that file into the presentation `<style>` before adding preset-specific theme tokens. Do not duplicate or maintain base viewport CSS in this file.
